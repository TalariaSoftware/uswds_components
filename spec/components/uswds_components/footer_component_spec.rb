...
RSpec.describe UswdsComponents::FooterComponent, type: :component do
  describe UswdsComponents::FooterComponent::SlimFooter, type: :component do
    ...
    it "is a footer" do
      render_component
      expect(page).to have_css('footer.usa-footer')
    end

    it "is a slim footer" do
      render_component
      expect(page).to have_css('footer.usa-footer--slim')
    end
    ...
    describe "primary section" do
      it "has a primary section" do
        render_component
        expect(page)
          .to have_css('footer > div.usa-footer__primary-section')
      end
      ...
      it "has a container that is a grid row" do
        render_component
        expect(page).to have_css('.usa-footer__primary-container.grid-row')
      end
  ...
  describe UswdsComponents::FooterComponent::Logo, type: :component do
    ...
    it "is a footer logo" do
      render_component
      expect(page).to have_css('div.usa-footer__logo')
    end

    it "is a grid row" do
      render_component
      expect(page).to have_css('.usa-footer__logo.grid-row')
    end

    it "has a 2 unit gap" do
      render_component
      expect(page).to have_css('.usa-footer__logo.grid-gap-2')
    end

    it "contains two grid columns" do
      render_component
      expect(page)
        .to have_css('.usa-footer__logo .grid-col-auto + .grid-col-auto')
    end
    ...
    it "point to the image file" do
      render_component
      expect(page).to have_css('img[src="/fake/logo.png"]')
    end
    ...
    context "when the logo src is blank" do
      ...
      it "does not have multiple grid column" do
        render_component
        expect(page).to have_no_css('.grid-col-auto + .grid-col-auto')
      end
  ...
  describe UswdsComponents::FooterComponent::PrimaryLink, type: :component do
    ...
    it "is a primary content list item" do
      render_component
      expect(page)
        .to have_css('li.usa-footer__primary-content')
    end

    it "is four columns wide on large mobile" do
      render_component
      expect(page)
        .to have_css('.usa-footer__primary-content.mobile-lg\:grid-col-4')
    end

    it "is auto width on desktop" do
      render_component
      expect(page)
        .to have_css('.usa-footer__primary-content.desktop\:grid-col-auto')
    end

    it "contains a primary link" do
      render_component
      expect(page).to have_css('li a.usa-footer__primary-link')
    end
  ...
  describe UswdsComponents::FooterComponent::ContactLink, type: :component do
    ...
    it "is auto width" do
      render_component
      expect(page).to have_css('div.grid-col-auto')
    end

    it "contains a link" do
      render_component
      expect(page).to have_css('div a')
    end
